...
public:
   ...
   */
  void send_batch(const std::string& reference_tag, const std::string& type, const std::vector<std::string>& parameters, std::vector<IrcMessage>& messages, MessageCallback callback);
  /**
...
private:
  void cap_end();
  void process_batch(const std::string& reference_tag);
public:
#ifdef WITH_SASL
  ...
  void on_batch(const IrcMessage& message);
...
private:
  ...
  std::unordered_map<std::string, std::pair<std::pair<std::string, std::vector<std::string>>, std::vector<IrcMessage>>> batches;
};
